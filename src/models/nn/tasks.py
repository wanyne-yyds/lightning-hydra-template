...
from src.utils import LOGGER, change_config, emojis
from src.utils.loss import v8ClassificationLoss, v8DetectionLoss, v8OBBLoss, v8PoseLoss, v8SegmentationLoss
from src.utils.plotting import feature_visualization
...
class MultiModel(MultiBaseModel):
    ...
    def load(self, weights):
        ...
        Args:
            ...
            verbose (bool, optional): Whether to log the transfer progress. Defaults to True.
        """
        csd = weights['state_dict']  # checkpoint state_dict as FP32
        csd = intersect_dicts(csd, self.state_dict())  # intersect
        self.load_state_dict(csd, strict=False)  # load
        return csd
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg="yolov8n.yaml", ch=3, nc=None, verbose=True):  # model, input channels, number of classes
        ...
        # Define model
        change_config(self.yaml, "ch", self.yaml.get("ch", ch))
        ch = self.yaml["ch"]  # input channels
...
def check_suffix(file="yolov8n.pt", suffix=".ckpt", msg=""):
...
def torch_safe_load(weight):
    ...
    check_suffix(file=weight, suffix=".ckpt")
...
def guess_model_task(model):
    ...
    def cfg2task(cfg):
        ...
        if cfg['head'][-1][-1][0] != 'nc':
            n = int(cfg['head'][-1][-1][0])
        else:
            n = 0
